import 'package:flutter/material.dart';
import 'package:final_grad_proj/core/app_export.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

class AppDecoration {
  static BoxDecoration get gradientBluegray401Bluegray600 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            0.5,
            -3.0616171314629196e-17,
          ),
          end: Alignment(
            0.5,
            0.9999999999999999,
          ),
          colors: [
            ColorConstant.bluegray401,
            ColorConstant.bluegray600,
          ],
        ),
      );
  static BoxDecoration get outlineBlack9000f => BoxDecoration(
        color: ColorConstant.whiteA700,
        boxShadow: [
          BoxShadow(
            color: ColorConstant.black9000f,
            spreadRadius: 2.00.r,
            blurRadius: 2.00.r,
            offset: Offset(
              0,
              0,
            ),
          ),
        ],
      );
  static BoxDecoration get gradientLightblue300BlueA200 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            0.5,
            -3.0616171314629196e-17,
          ),
          end: Alignment(
            0.5,
            0.9999999999999999,
          ),
          colors: [
            ColorConstant.lightBlue300,
            ColorConstant.blueA200,
          ],
        ),
      );
  static BoxDecoration get gradientWhiteA70099IndigoA40099 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            0.5,
            -3.0616171314629196e-17,
          ),
          end: Alignment(
            0.5,
            0.9999999999999999,
          ),
          colors: [
            ColorConstant.whiteA70099,
            ColorConstant.indigoA40099,
          ],
        ),
      );
  static BoxDecoration get gradientIndigoA400Gray200 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            -4.593806657987587e-9,
            0.44766010769375564,
          ),
          end: Alignment(
            0.9999999901829075,
            0.45566502208543164,
          ),
          colors: [
            ColorConstant.indigoA400,
            ColorConstant.gray200,
          ],
        ),
      );
  static BoxDecoration get outlineBlack9000f1 => BoxDecoration(
        color: ColorConstant.whiteA700,
        boxShadow: [
          BoxShadow(
            color: ColorConstant.black9000f,
            spreadRadius: 2.00.r,
            blurRadius: 2.00.r,
            offset: Offset(
              0,
              -1,
            ),
          ),
        ],
      );
  static BoxDecoration get fillBlack90087 => BoxDecoration(
        color: ColorConstant.black90087,
      );
  static BoxDecoration get outlineBlack9000c => BoxDecoration(
        color: ColorConstant.whiteA700,
        boxShadow: [
          BoxShadow(
            color: ColorConstant.black9000c,
            spreadRadius: 2.00.r,
            blurRadius: 2.00.r,
            offset: Offset(
              0,
              0,
            ),
          ),
        ],
      );
  static BoxDecoration get outlineBlack9000c1 => BoxDecoration(
        color: ColorConstant.whiteA700,
        boxShadow: [
          BoxShadow(
            color: ColorConstant.black9000c,
            spreadRadius: 2.00.r,
            blurRadius: 2.00.r,
            offset: Offset(
              0,
              6,
            ),
          ),
        ],
      );
  static BoxDecoration get gradientTealA400TealA700 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            0.5,
            -3.0616171314629196e-17,
          ),
          end: Alignment(
            0.5,
            0.9999999999999999,
          ),
          colors: [
            ColorConstant.tealA400,
            ColorConstant.tealA700,
          ],
        ),
      );
  static BoxDecoration get fillGray51 => BoxDecoration(
        color: ColorConstant.gray51,
      );
  static BoxDecoration get fillGray52 => BoxDecoration(
        color: ColorConstant.gray52,
      );
  static BoxDecoration get fillIndigoA400 => BoxDecoration(
        color: ColorConstant.indigoA400,
      );
  static BoxDecoration get fillGray50 => BoxDecoration(
        color: ColorConstant.gray50,
      );
  static BoxDecoration get outlineBlack900142 => BoxDecoration(
        color: ColorConstant.whiteA700,
        boxShadow: [
          BoxShadow(
            color: ColorConstant.black90014,
            spreadRadius: getHorizontalSize(
              2.00,
            ),
            blurRadius: getHorizontalSize(
              2.00,
            ),
            offset: Offset(
              0,
              0,
            ),
          ),
        ],
      );
  static BoxDecoration get gradientRed200PinkA100 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            0.5,
            -3.0616171314629196e-17,
          ),
          end: Alignment(
            0.5,
            0.9999999999999999,
          ),
          colors: [
            ColorConstant.red200,
            ColorConstant.pinkA100,
          ],
        ),
      );
  static BoxDecoration get outlineBlack90014 => BoxDecoration(
        color: ColorConstant.whiteA700,
        boxShadow: [
          BoxShadow(
            color: ColorConstant.black90014,
            spreadRadius: 2.00.r,
            blurRadius: 2.00.r,
            offset: Offset(
              0,
              0,
            ),
          ),
        ],
      );
  static BoxDecoration get outlineBlack900141 => BoxDecoration(
        color: ColorConstant.whiteA700,
        boxShadow: [
          BoxShadow(
            color: ColorConstant.black90014,
            spreadRadius: 2.00.r,
            blurRadius: 2.00.r,
            offset: Offset(
              0,
              0,
            ),
          ),
        ],
      );
  static BoxDecoration get gradientIndigoA400IndigoA401 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            0.4999999653693671,
            1.7440376530106216e-10,
          ),
          end: Alignment(
            0.5013333310534325,
            1.1620112127457618,
          ),
          colors: [
            ColorConstant.indigoA400,
            ColorConstant.indigoA401,
          ],
        ),
      );
  static BoxDecoration get gradientOrange200DeeporangeA200 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            0.5,
            -3.0616171314629196e-17,
          ),
          end: Alignment(
            0.5,
            0.9999999999999999,
          ),
          colors: [
            ColorConstant.orange200,
            ColorConstant.deepOrangeA200,
          ],
        ),
      );
  static BoxDecoration get outlineBluegray90019 => BoxDecoration(
        color: ColorConstant.whiteA700,
        border: Border.all(
          color: ColorConstant.bluegray90019,
          width: 1.00.w,
        ),
      );
  static BoxDecoration get txtOutlineBluegray50028 => BoxDecoration(
        color: ColorConstant.whiteA700,
        border: Border.all(
          color: ColorConstant.bluegray50028,
          width: 0.80.w,
        ),
      );
  static BoxDecoration get fillWhiteA701 => BoxDecoration(
        color: ColorConstant.whiteA701,
      );
  static BoxDecoration get fillWhiteA700 => BoxDecoration(
        color: ColorConstant.whiteA700,
      );
  static BoxDecoration get gradientGray600Bluegray902 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            0.5,
            -3.0616171314629196e-17,
          ),
          end: Alignment(
            0.5,
            0.9999999999999999,
          ),
          colors: [
            ColorConstant.gray600,
            ColorConstant.bluegray902,
          ],
        ),
      );
  static BoxDecoration get gradientIndigoA40099WhiteA70099 => BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment(
            0.5042016749202367,
            0.9999999529700284,
          ),
          end: Alignment(
            0.5042016734942809,
            -4.7029642180618225e-8,
          ),
          colors: [
            ColorConstant.indigoA40099,
            ColorConstant.whiteA70099,
          ],
        ),
      );
  static BoxDecoration get outlineBluegray50028 => BoxDecoration(
        border: Border.all(
          color: ColorConstant.bluegray50028,
          width: 1.00.w,
        ),
      );
  static BoxDecoration get fillTealA70014 => BoxDecoration(
        color: ColorConstant.tealA70014,
      );
}

class BorderRadiusStyle {
  static BorderRadius customBorderTL30 = BorderRadius.only(
    topLeft: Radius.circular(
      30.00.r,
    ),
    topRight: Radius.circular(
      30.00.r,
    ),
  );

  static BorderRadius customBorderTL12 = BorderRadius.only(
    topLeft: Radius.circular(
      12.00.r,
    ),
    topRight: Radius.circular(
      12.00.r,
    ),
    bottomLeft: Radius.circular(
      2.00.r,
    ),
    bottomRight: Radius.circular(
      2.00.r,
    ),
  );

  static BorderRadius txtRoundedBorder12 = BorderRadius.circular(
    12.00.r,
  );

  static BorderRadius customBorderBL20 = BorderRadius.only(
    bottomLeft: Radius.circular(
      20.00.r,
    ),
    bottomRight: Radius.circular(
      20.00.r,
    ),
  );

  static BorderRadius customBorderBL30 = BorderRadius.only(
    bottomLeft: Radius.circular(
      30.00.r,
    ),
    bottomRight: Radius.circular(
      30.00.r,)
  );

  static BorderRadius roundedBorder6 = BorderRadius.circular(
    6.00.r,
  );

  static BorderRadius circleBorder25 = BorderRadius.circular(
    25.00.r,
  );

  static BorderRadius roundedBorder12 = BorderRadius.circular(
    12.00.r,
  );

  static BorderRadius roundedBorder2 = BorderRadius.circular(
    2.50.r,
  );

  static BorderRadius roundedBorder32 = BorderRadius.circular(
    32.00.r,
  );

  static BorderRadius circleBorder42 = BorderRadius.circular(
    42.00.r,
  );

  static BorderRadius circleBorder21 = BorderRadius.circular(
    21.00.r,
  );

  static BorderRadius circleBorder65 = BorderRadius.circular(
    65.00.r,
  );

  static BorderRadius customBorderTL301 = BorderRadius.only(
    topLeft: Radius.circular(
      30.00.r,
    ),
    bottomLeft: Radius.circular(
      30.00.r,
    ),
  );
}
